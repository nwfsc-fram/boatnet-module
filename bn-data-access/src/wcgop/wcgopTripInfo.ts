...
import { mongoService } from '@boatnet/bn-mongo';
...
class WcgopTripImpl extends Base {
    ...
    async getTripsByEvaluationPeriod(evaluationPeriod: EvaluationPeriods) {
        if (this.type === 'mongo') {
            ...
            return await mongoService.read('masterdb', 'trips', queryParams);
        } else {
    ...
    async getTrips(database: string, collection: string, queryParams: any, options: any) {
        return await mongoService.read(database, collection, queryParams, options);
    }
}
